...
> **Codex 本机使用说明（Windows + Codex Desktop，插件 easyeda-bridge v1.1.0）**
> 本技能随插件分发，插件目录为 `C:\Users\34021\plugins\easyeda-bridge`。
> Codex 不提供 `CLAUDE_SKILL_DIR` 环境变量，一律使用下面的绝对路径：
>
> ```powershell
> # Bridge Server 通常无需手动启动：MCP 工具会在缺失时自动拉起；需要手动时：
> powershell -File C:\Users\34021\plugins\easyeda-bridge\scripts\start-bridge.ps1
>
> # 健康检查 / 已连接窗口 / 三态体检
> curl http://localhost:49620/health
> curl http://localhost:49620/eda-windows
> node C:\Users\34021\plugins\easyeda-bridge\scripts\check.mjs
> ```
>
> **优先走 MCP 工具**（`easyeda_bridge_status` / `easyeda_list_windows` / `easyeda_select_window` / `easyeda_execute`）；
> 只有需要查 API 文档时才用本技能。EDA 侧对应扩展是 **Run API Gateway**，保持「已启用 + 允许外部交互」。
>
> ### 实测注意（v1.1.0 验证，EDA 3.2.149 / 扩展 v1.0.5）
> 1. **先探再调**：`return Object.keys(eda.dmt_Xxx)` 拿真实方法名再按签名传参，勿零参盲调（多数 getter 需要入参）。
> 2. **先开文档**：访问 `sch_*` / `pcb_*` 前先 `await eda.dmt_EditorControl.openDocument(uuid)`（uuid 取自 `dmt_Project.getCurrentProjectInfo().data[].schematic.uuid / .pcb.uuid`），否则报 `Cannot read properties of null (reading 'map')` / `指定的主题消息在对应的画布内没有相关订阅`。
> 3. **写操作闭环**：create → get 复核 → 需要时 delete 复原 → `save()`。
> 4. **避坑**：`sys_FileSystem` 的 `getDocumentsPath` / `getLibrariesPaths` / `getProjectsPaths` / `listFilesOfFileSystem` 会阻塞不返回（26s 实测），需短超时或避开；`pcb_ManufactureData.getManufactureData` 仅私有化部署版有效。
